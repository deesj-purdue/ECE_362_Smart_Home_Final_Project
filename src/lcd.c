...
#define RESET_HIGH do { GPIOA->BSRR = GPIO_BSRR_BS_11; } while(0)
#define RESET_LOW  do { GPIOA->BSRR = GPIO_BSRR_BR_11; } while(0)
#define DC_NUM 10
...
void init_lcd_spi() {
    ...
    //PA5 and 7 to AF0 for SCK and MOSI
    ...
    //Set PA4, 10, and 11 as outputs for CS, nRESET, and DC
    GPIOA->MODER &= ~(GPIO_MODER_MODER4 | GPIO_MODER_MODER10 | GPIO_MODER_MODER11);
    GPIOA->MODER |= (GPIO_MODER_MODER4_0 | GPIO_MODER_MODER10_0 | GPIO_MODER_MODER11_0);

    /* SPI Configuration */
    SPI1->CR1 &= ~SPI_CR1_BR; //Baud rate divisor = 2 to get 24 MHz
    ...
    SPI1->CR1 |= SPI_CR2_FRXTH; //SPI channel releases 8-bit value when FIFO level hits 8-bits
    SPI1->CR1 |= SPI_CR1_SPE;
}
...
//===========================================================================
static void _LCD_DrawPoint(uint16_t x, uint16_t y, uint16_t c)
{
...
//===========================================================================
static void _LCD_DrawLine(uint16_t x1, uint16_t y1, uint16_t x2, uint16_t y2, uint16_t c)
{
    uint16_t t;
...
//===========================================================================
static void _LCD_Fill(uint16_t sx,uint16_t sy,uint16_t ex,uint16_t ey,uint16_t color)
{
    uint16_t i,j;
...
//===========================================================================
void _LCD_DrawChar(uint16_t x,uint16_t y,uint16_t fc, uint16_t bc, char num, uint8_t size, uint8_t mode)
{
    uint8_t temp;
    uint8_t pos,t;
    num=num-' ';
